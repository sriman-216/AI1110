...
Let the event that difference between the numbers on the dice is 2 be $E$.

Let us assume $X_1$ be the outcome of first die and $X_2$ be the outcome of second die

To satisfy the event E
\begin{align}
    X_1 - X_2 &= 2 \label{eq:xy} \\
    \text{(or)} \nonumber \\
    X_2 - X_1 &= 2 \label{eq:xy2}
\end{align}
...
Consider $E_1$
\begin{align}
    Pr(E_1) &= Pr (X_1-X_2 = 2) = Pr (X_1 = X_2+2 )\nonumber\\
   	&=\sum_{k} Pr(X_1=k+2|X_2=k)p_{X_{2}}(k)\label{eq:3}
\end{align}
after unconditioning. $X_1$ and $X_2$ are independent,
\begin{align}
	Pr(X_1=k+2|X_2=k)&=Pr(X_1=k+2)\nonumber\\
                &=p_{X_{1}}(k+2)\label{eq:4}
\end{align} 

From \eqref{eq:3} and \eqref{eq:4},
\begin{align}
  Pr(E_1) &=\sum_{k}p_{X_{1}}(k+2)p_{X_{2}}(k)
  &= p_{X_{1}}(2)*p_{X_{2}}(2)\label{eq:5}
\end{align}
where * denotes the convolution operation. Substituting from \eqref{eq:1} in \eqref{eq:4}
\begin{align}
  Pr(E_1) &=\frac{1}{6}\sum_{k=1}^{6}p_{X_{1}}(k+2)=\frac{1}{6}\sum_{3}^{8}p_{X_{1}}(k)\label{eq:6}
\end{align}
Substituting from \eqref{eq:1} in \eqref{eq:6}
\begin{align}
  Pr(E_1) &=\frac{1}{6}\sum_{3}^{8}p_{X_{1}}(k)\nonumber\\
  &=\frac{1}{6}(\sum_{3}^{6}p_{X_{1}}(k)+\sum_{7}^{8}p_{X_{1}}(k))\nonumber\\
   &=\frac{1}{6}(4\frac{1}{6}+0)\nonumber\\
    &=\frac{4}{36}\nonumber\\
\end{align}
...
Two dice are thrown at the same time the probability that the difference of the numbers on the two dice is 2 is \(\frac{8}{36}\)
